...
class Wxml2Canvas {
  constructor(options = {}) {
    ...
    this.destZoom = options.destZoom || this.device.pixelRatio
  ...
  _preloadImage(list = []) {
    ...
    const cacheImageInfo = (target, resolve, reject) => {
      wx.getImageInfo({
        src: target,
        success: res => {
          const index = this._findPicIndex(target)
          let tempFilePath = res.path

          if (!/^\W|^\w+:\/\//.test(tempFilePath)) {
            tempFilePath = '/' + tempFilePath
          }

          if (index > -1) {
            this.allPic[index].local = tempFilePath
            this.allPic[index].width = res.width
            this.allPic[index].height = res.height
          }
          resolve()
        },
        fail: err => {
    ...
    list.forEach(item => {
      // 避免重复下载同一图片
      if (!item.url || this._findPicIndex(item.url) > -1) return

      this.allPic.push({url: item.url, local: ''})
      ...
      all[count++] = new Promise((resolve, reject) => {
        ...
        if (
          ...
          // 注：数据方式传入 base64 图片时，定义 item.base64Img 非 item.url
          if (item.base64Img) {
            const fileManager = wx.getFileSystemManager()
            const filePath = item.url || this._genLocalFilePath()

            fileManager.writeFile({
              filePath,
              data: item.base64Img.replace(/data:image\/(.*);base64,/, ''),
              encoding: 'base64',
              success: res => {
                cacheImageInfo(filePath, resolve, reject)
              },
              fail: reject,
            })
          } else {
            cacheImageInfo(item.url, resolve, reject)
          }
        } else {
          cacheImageInfo(item.url, resolve, reject)
        }
  ...
  _genLocalFilePath() {
    ...
    return `${wx.env.USER_DATA_PATH}/${uid}.jpg`

    // 在插件中使用下面的格式
    // return `http://tmp/${uid}.jpg`
  }
  ...
  _drawRectToCanvas(x, y, width, height, style, url, mode) {
    ...
    const _border = utils.transferBorder(border) || {width: 0}
  ...
  _clipRadius(x, y, width, height, border, radius = 0) {
    ...
    if (radius === width) {
      ...
      ctx.arc(leftTop.x + radius, y + radius, radius - fixBorder, utils.deg2Rad(180), utils.deg2Rad(270))

      ctx.lineTo(rightTop.x - radius, rightTop.y + fixBorder)
      ctx.arc(rightTop.x - radius, rightTop.y + radius, radius - fixBorder, utils.deg2Rad(270), utils.deg2Rad(360))

      ctx.lineTo(rightBottom.x - fixBorder, rightBottom.y - radius)
      ctx.arc(rightBottom.x - radius, rightBottom.y - radius, radius - fixBorder, utils.deg2Rad(0), utils.deg2Rad(90))

      ctx.lineTo(leftBottom.x + radius, leftBottom.y - fixBorder)
      ctx.arc(leftBottom.x + radius, leftBottom.y - radius, radius - fixBorder, utils.deg2Rad(90), utils.deg2Rad(180))
  ...
  _drawText(item, style, resolve, reject, type, isWxml) {
    ...
    try {
      ...
      let text = item.text || ''
      let textWidth = Math.floor(this._measureWidth(text, style.font || this.ctx.font))
      ...
      if (typeof style.padding === 'string') {
        style.padding = utils.transferPadding(style.padding)
      }
      item.x = this._resetPositionX(item, style)
      ...
      if (type === 'inline-text') {
        ...
        if (item.leftOffset + textWidth > width) {
          ...
          textWidth = Math.floor(this._measureWidth(text, style.font || this.ctx.font))
          ...
          const lastWidth = this._measureWidth(last)
        ...
        if (width && textWidth > width && whiteSpace !== 'nowrap') {
          ...
          let lastWidth = this._measureWidth(last)
          if (lastWidth > width) {
  ...
  _formatImage(res = []) {
    ...
    res.forEach((item, index) => {
      ...
      const filePath = this._genLocalFilePath()
      let sub = {}
      if ((dataset.type === 'image' || dataset.type === 'radius-image') && dataset.url) {
        sub = {
          url: dataset.base64 ? filePath : dataset.url,
          base64Img: dataset.base64 ? dataset.url : '',
        }
        ...
        sub = {
          url: dataset.base64 ? filePath : url,
          base64Img: dataset.base64 ? url : '',
        }
      }
  ...
  _transferWxmlStyle(sub, item, limitLeft, limitTop) {
    ...
    if (typeof padding === 'string') {
      padding = utils.transferPadding(padding)
    }

    const paddingTop = +padding[0] + this._parseNumber(sub.paddingTop)
    const paddingRight = +padding[1] + this._parseNumber(sub.paddingRight)
    const paddingBottom = +padding[2] + this._parseNumber(sub.paddingBottom)
    const paddingLeft = +padding[3] + this._parseNumber(sub.paddingLeft)

    sub.padding = [paddingTop, paddingRight, paddingBottom, paddingLeft]

    // 新增
    sub.borderTopLeftRadius = this._parseNumber(sub.borderTopLeftRadius)
    sub.borderTopRightRadius = this._parseNumber(sub.borderTopRightRadius)
    sub.borderBottomRightRadius = this._parseNumber(sub.borderBottomRightRadius)
    sub.borderBottomLeftRadius = this._parseNumber(sub.borderBottomLeftRadius)
  ...
  _getTextSingleLine(text, width, singleLength, currentIndex = 0, widthOffset = 0) {
    ...
    let singleWidth = this._measureWidth(single)
    ...
    while (Math.round(widthOffset + singleWidth) > width) {
      ...
      singleWidth = this._measureWidth(single)
    }
  ...
  _drawBorder(border, style, callback) {
    ...
    border = utils.transferBorder(border)
  ...
  _drawBoxShadow(boxShadow, callback) {
    boxShadow = utils.transferBoxShadow(boxShadow)
    if (boxShadow) {
