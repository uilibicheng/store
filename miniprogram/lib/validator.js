...
export default class Validator {
    ...
    that.methods = {
      ...
      email(value) {
        return (
          ...
          that.optional(value) || /^[a-zA-Z0-9.!#$%&'*+\/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/.test(value)
        )
      },
      /**
       * 验证手机格式
      ...
      url(value) {
        return (
          // prettier-ignore
          // eslint-disable-next-line
          that.optional(value) || /^(?:(?:(?:https?|ftp):)?\/\/)(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})).?)(?::\d{2,5})?(?:[/?#]\S*)?$/i.test(value)
        )
      },
      /**
       * 验证日期格式
  ...
  removeMethod(name) {
    ...
    delete this.defaults.messages[name]
  }
   ...
   */
  checkParam(param, data) {
    if (!this.hasView(param)) this.setView(param)

    // 去除上一次验证结果
    const errorListIndex = this.errorList.findIndex(item => item.param === param)
    if (errorListIndex > -1) this.errorList.splice(errorListIndex, 1)
    ...
    for (const method in rules) {
      ...
      if (this.isValidMethod(method)) {
        ...
        validateResult = this.setValue(param, method, result, value)

        // 判断是否通过验证，否则缓存错误信息，跳出循环
        if (!result) {
          this.formatTplAndAdd(param, rule, value)
          validateResult.$msg = this.defaultMessage(param, rule)
          break
        }
      }
    }
